...
class CommentaryOutput(BaseModel):
    matchSummary: str
    keyPlayerInsight: Optional[str] = None
...
class Inference:
    def __init__(self):
        ...
        match_models = glob.glob(os.path.join(project_root, "models", "*_matches_model.pkl"))
        if not match_models:
            raise FileNotFoundError(f"No match prediction model file (*_matches_model.pkl) found in {os.path.join(project_root, 'models')}")
        self.model_path = match_models[0]
        self.features_path = os.path.join(project_root, "features", "match_trained_features.pkl")
        
        # Dynamically locate player regression model
        player_models = glob.glob(os.path.join(project_root, "models", "*_players_model.pkl"))
        if not player_models:
            raise FileNotFoundError(f"No player performance model file (*_players_model.pkl) found in {os.path.join(project_root, 'models')}")
        self.player_model_path = player_models[0]
        self.player_features_path = os.path.join(project_root, "features", "player_trained_features.pkl")
    ...
    def test_match_infer(self, data: MatchInput) -> MatchOutput:
        ...
        match_features_ctx = MatchFeaturesContext(
            eloComparison=EloComparison(homeElo=round(float(hs['elo']), 2), awayElo=round(float(aws['elo']), 2)),
            formComparison=FormComparison(homeWeightedForm=round(float(home_wf), 2), awayWeightedForm=round(float(away_wf), 2)),
            attackStrength=StrengthComparison(home=round(float(home_attack), 2), away=round(float(away_attack), 2)),
            defenseStrength=StrengthComparison(home=round(float(home_defense), 2), away=round(float(away_defense), 2)),
            cleanSheetRate=StrengthComparison(home=round(float(home_clean_sheet_rate), 2), away=round(float(away_clean_sheet_rate), 2)),
            headToHead=HeadToHeadStats(matchesPlayed=int(h2h['matches']), homeWinRate=round(float(h2h_home_win_rate), 2))
        )
    ...
    def test_player_infer(self, match_pred: MatchOutput) -> MatchReportResponse:
        ...
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        historical_df = pd.read_csv(os.path.join(project_root, "data", "players_ml.csv"))

        # Process dates
        historical_df['startDate'] = pd.to_datetime(
            historical_df['startDate'],
            errors='coerce'
        )
        historical_df = historical_df.sort_values('startDate')
        ...
        winning_players_df = winning_players_df.sort_values('roll5_perf', ascending=False).head(8)
        ...
        if winning_players_df.empty or not valid_features:
            winning_players_df['predicted_perf_score'] = 0.0
            winning_players_df['play_well_probability'] = 0.0
        else:
            ...
            if not match_players or player_name in match_players:
                ...
                insight = PlayerInsight(
                    ...
                    career=CareerStats(
                        ...
                        goalsPerGame=round(safe_float(row.get('career_avg_goals', 0.0)), 2)
                    ),
                    recentForm=RecentFormStats(
                        roll5PerfScore=safe_float(row.get('roll5_perf', 0.0)),
                    ...
                    experienceScore=round(safe_float(row.get('log_experience', 0.0)), 2),
                    clutchGoals=safe_int(row.get('career_best_player_rate', 0))
                )
