...
class NarrativeContextBuilder:
    ...
    def _prepare_players(self):
        ...
        if 'startDate' in p.columns and 'match_date' not in p.columns:
            p.rename(columns={'startDate': 'match_date'}, inplace=True)
            
        if 'match_date' in p.columns:
        ...
        if 'player_name' not in p.columns:
            if 'first_name' in p.columns and 'last_name' in p.columns:
                p['player_name'] = (p['first_name'].fillna('') + ' ' + p['last_name'].fillna('')).str.strip()
            else:
                p['player_name'] = 'Unknown Player'
                
        if 'SCORE' in p.columns and 'goals' not in p.columns:
            p.rename(columns={'SCORE': 'goals'}, inplace=True)
...
class PromptBuilder:
    ...
    def build_system(self, has_player: bool) -> str:
        base = """You are a professional football analyst.
Explain the likely outcome of the match using the provided historical JSON data.

Rules:
- Interpret statistics instead of simply repeating them. Prefer insights.
- Focus on momentum, consistency, attacking/defensive strength, and player impact.
- Do not mention AI, ML, probabilities, models, or datasets.
- Keep explanations concise.

Output format: Return a valid JSON object with EXACTLY the following keys:
"""
        if has_player:
            base += """{
  ...
  "overallAssessment": "<1-2 sentences>"
    ...
    def build(self, ctx: dict) -> str:
        # We drop "all_players" as it contains too much unused data that consumes tokens
        llm_ctx = {k: v for k, v in ctx.items() if k != "all_players"}
        
        # If best player is N/A, we don't need player_contexts
        if llm_ctx.get("best_player") == "N/A":
            llm_ctx.pop("player_contexts", None)
            
        return json.dumps(llm_ctx, indent=2)
...
class CommentaryGenerator:
    ...
    async def generate(self, system_prompt: str, user_prompt: str) -> dict:
        ...
        # Wait a small fixed amount before every request to gracefully respect ~15 RPM free tier limits
        ...
        for attempt in range(max_attempts):
            try:
                ...
                print(f"[CommentaryGenerator] Attempt {attempt + 1}/{max_attempts} failed: {e}")
                if attempt < max_attempts - 1:
                    delay = base_delay * (2 ** attempt)  # Exponential backoff: 4, 8, 16, 32...
                    print(f"[CommentaryGenerator] Waiting {delay} seconds before retrying...")
...
async def enrich_prediction_with_commentary(
    ...
    system_prompt  = prompt_builder.build_system(context["best_player"] != "N/A")
